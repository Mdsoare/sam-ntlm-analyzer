...
function showUIFeedback(message, type = "warning") {
  let feedbackBanner = document.getElementById("uiFeedbackBanner");

  if (!feedbackBanner) {
    feedbackBanner = document.createElement("div");
    feedbackBanner.id = "uiFeedbackBanner";
    feedbackBanner.setAttribute("role", "alert");

    const mainContainer = document.querySelector("main") || document.body;
    mainContainer.insertBefore(feedbackBanner, mainContainer.firstChild);
  }
...
function clearUIFeedback() {
  const feedbackBanner = document.getElementById("uiFeedbackBanner");
  if (feedbackBanner) {
    feedbackBanner.style.display = "none";
  }
}
...
function parseSamData() {
  ...
  if (!rawInput.trim()) {
    ...
    showUIFeedback(
      "Por favor, forneça strings estruturadas obtidas a partir de ferramentas de dump ou relatórios de auditoria SAM.",
      "warning",
    );
    return;
  }
  ...
  if (validRowsCount > 0) {
    ...
    resultsSection.style.display = "none";
    showUIFeedback(
      "Nenhuma linha compatível com a sintaxe padrão de dump (usuário:RID:LM:NT) foi identificada.",
      "danger",
    );
  }
}
